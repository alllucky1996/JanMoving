...
namespace Smartstore.Moving.Services
{
    internal class NewsItemConverter
    {
        ...
        public async Task<List<VideoItem>> ImportAllAsync()
        {
            var newsImported = new List<VideoItem>();
            var table = _db.Set<VideoItem>();
        ...
        public async IAsyncEnumerable<VideoItem> LoadAllAsync()
        {
        ...
        private Task<VideoItem> DeserializeNews(string fullPath)
            => DeserializeDocument(XDocument.Load(fullPath));

        private async Task<VideoItem> DeserializeDocument(XDocument doc)
        {
